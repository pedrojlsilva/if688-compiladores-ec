...
public class Interpreter implements Expr.Visitor<Integer> {
	...
	@Override
	public Integer visitVarExpr(Expr.Var expr) {
		if(env.get(expr.value.toString()) != null) {
			return Integer.parseInt(env.get(expr.value.toString()));
			
		}
		throw new EnvError("Var " + expr.value.toString() + " is not defined");
